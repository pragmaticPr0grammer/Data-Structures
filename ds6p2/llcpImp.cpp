...
{
	if (xHead == 0 && yHead == 0)	// base case (both empty)
		return;

	if (xHead == 0)		// xhead empty but yhead not
	{
		zHead = yHead;
		yHead = yHead->link;
	...
	else if (yHead == 0)	// yHead empty buy xHead not
	{
		zHead = xHead;
		xHead = xHead->link;
	}

	else if (xHead->data <= yHead->data)	//x AND y not empty
	{
		zHead = xHead;
		xHead = xHead->link;
	}
